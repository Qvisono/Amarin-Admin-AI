...
Amarin Admin AI — локальный консольный агент, который понимает задачи на естественном языке и **сам выполняет** действия на вашем Windows-ПК через набор из 30+ инструментов.

Вместо Диспетчера устройств, Event Viewer, `services.msc` и ручного PowerShell — просто опишите проблему:
...
Агент собирает данные, при необходимости ищет решение в сети, применяет исправления (опасные шаги — только с вашим подтверждением) и кратко отчитывается. Большая часть кода проекта написана с использованием ИИ.

|                | |
|:---------------|:--|
| **Платформа**  | Windows 10 / 11 (x64) |
| **Стек**       | .NET 10, Spectre.Console, Markdig, PDFsharp |
| **Модели**     | Venice.ai (по умолчанию `grok-4-5`, смена через `/model`) |
...
**Требования:** Windows 10/11 (x64) · API-ключ [Venice.ai](https://venice.ai) · для сборки — [.NET 10 SDK](https://dotnet.microsoft.com/download)

### Готовый релиз

1. Скачайте архив из [`release/`](release/) (например `Amarin-Admin-AI-v1.2.0-win-x64.zip`)
2. Распакуйте в любую папку
3. Укажите API-ключ (см. ниже)
...
> ⚠️ Не коммитьте реальный ключ в git — в репозитории он должен оставаться пустым или задаваться только локально.
...
- **Диалог с инструментами** — модель планирует и вызывает tools в несколько раундов (`MaxToolRounds`)
- **Два режима сессии** (`/session`) — с историей контекста или каждый запрос с чистого листа
- **Read-only режим** (`/readonly`) — запись и опасный PowerShell блокируются
...
- **Палитра команд** — введите `/` для поиска и выбора
...
<details>
<summary><b>🖥 Система и диагностика</b> — 8 инструментов</summary>
...
<details>
<summary><b>⚙️ Процессы, службы, автозагрузка</b> — 5 инструментов</summary>
...
<details>
<summary><b>📁 Файлы, реестр, PowerShell</b> — 5 инструментов</summary>
...
<details>
<summary><b>🌐 Сеть и удалённый доступ</b> — 5 инструментов</summary>
...
<details>
<summary><b>🔄 Обновления Windows</b> — 1 инструмент</summary>
...
<details>
<summary><b>👤 Пользовательский контекст и веб</b> — 6 инструментов</summary>
...
- Нет управления GUI (кликов по окнам) — задачи решаются через tools и PowerShell
- Для полного доступа к службам, драйверам и системным журналам нужен запуск **от администратора**
