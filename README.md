...
**2. Отдайте программе ключ Venice.** Поля для ключа в интерфейсе нет намеренно: тогда его
пришлось бы где-то хранить. Ключ читается из переменной окружения — она принадлежит вашей
учётной записи Windows и другим пользователям компьютера не видна.

```powershell
[Environment]::SetEnvironmentVariable('VENICE_API_KEY', 'сюда-ваш-ключ', 'User')
```

Переменные читаются один раз при старте, поэтому после этой команды программу надо перезапустить.
