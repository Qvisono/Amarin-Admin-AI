...
public sealed class Wave7ChatTests
{
    ...
    public void Title_sanitizer_truncates_long_line()
    {
        ...
        var prompt = ChatTitle.SystemPrompt("Русский");
        Assert.Contains("24", prompt, StringComparison.Ordinal);
        Assert.Contains("not a chatbot", prompt, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("Never answer", prompt, StringComparison.Ordinal);
    }

    [Fact]
    public void Title_user_prompt_wraps_the_message_so_it_is_not_answered()
    {
        var wrapped = ChatTitle.UserPrompt("привет", "Русский");
        ...
        // Сообщение по-прежнему в заборе из «---», а за ним — напоминание про язык: на длинном
        // первом сообщении инструкция из системного промпта тонет.
        Assert.Contains("---", wrapped, StringComparison.Ordinal);
        Assert.EndsWith("Write only the title, in Русский.", wrapped, StringComparison.Ordinal);
    }
